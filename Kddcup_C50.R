...
### C5.0 ###
fit.c50 <- train(vec2~., data=trainSplit, method="C5.0", metric=metric, trControl=control)
### Testing C50 ####
c50Pred<-predict(fit.c50,testSplit)
a1<-confusionMatrix(c50Pred,testSplit$vec2)
...
saveRDS(fit.c50, "/home/charan/Desktop/Rcode/final_model_Ids_c50.rds")
#CHECKINH MODEL IS WORKING PROPERLY OR NOT
super_model <- readRDS("./final_model_Ids_c50.rds")
...
### gbm ###   
fit.gbm <- train(vec2~., data=trainSplit, method="gbm", metric=metric, trControl=control)
### Testing SVM ####
gbmPred<-predict(fit.gbm,testSplit)
a2<-confusionMatrix(gbmPred,testSplit$vec2)
print(sprintf("gbm Results for Iteration: %d", i))
...
### RF ###
fit.rf <- train(vec2~., data=trainSplit, method="rf", metric=metric, trControl=control)
### Testing cart ####
rfPred<-predict(fit.rf,testSplit)
a3<-confusionMatrix(rfPred,testSplit$vec2)
print(sprintf("linear svm Results for Iteration: %d", i))
